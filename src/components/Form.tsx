...
export function Form() {
  ...
  return (
    <div className="w-full max-w-[900px] p-10">
      ...
      >
        <div className="flex flex-col w-full relative mt-[20px]">
          <input
            ...
            type="text"
          />
          <label className="pointer-events-none  absolute top-[0px] transition-[0.3s] peer-valid:top-[-20px] peer-valid:text-[#7192a2] peer-valid:font-[500] peer-valid:text-[14px]" htmlFor="">
            Your Name
          </label>
        </div>
        <div className="flex flex-col w-full relative">
          <input
            ...
            type="text"
          />
          <label className="pointer-events-none  absolute top-[0px] peer-valid:top-[-20px] peer-valid:text-[#7192a2] peer-valid:font-[500] peer-valid:text-[14px]" htmlFor="">
            Your Email
          </label>
        </div>
        ...
        <textarea ref={formMessage} name="entry.1937766715" className="bg-transparent outline-none inset-3 shadow-[inset_0_0_12px_#0004] rounded-[4px] resize-none h-[200px] p-[10px] text-inherit" />
        <button ref={formBtnSend} className="bg-[#121212] text-[#fff] p-3" onClick={async (e) => {
          e.preventDefault()
          if (formUsername.current.value.length > 2 && formEmail.current.value.length > 4 && formMessage.current.value.length > 0) {
      ...
      <iframe src="https://docs.google.com/forms/u/0/d/e/1FAIpQLSfDbayAADAu06mZo2VSV5nJvENCnEJlqOVhkxL9mE2D8yNXEg/formResponse" name="iframeForm" style={{ display: "none" }} id="iframeForm" />
    </div>
  )
}
