...
export function ButtonChoice({ children, interaction, dropDownOpen, setDropDownOpen }) {
...
export const WordIcon = () => { return (<BiWorld className="text-[16px]" />) }
export const ConfigIcon = () => { return (<GoGear className="text-[16px]" />) }

export function ItemMenuDropdown({ label, type, interaction = "hover" }: ItemMenuDropdownTypes) {
    ...
    <>
      <div>
        <div className="relative flex justify-center items-center gap-2 text-left">
          <ButtonChoice interaction={interaction} dropDownOpen={dropDownOpen} setDropDownOpen={setDropDownOpen}>
            {label && <p className="mr-[5px]">{label}</p>}
            {type === 'lang' && <WordIcon />}
            {type === 'config' && <GoGear />}
            <TfiAngleDown className="text-[13px] text-[#565656]" />
          </ButtonChoice>
          <div className="absolute w-fit h-fit top-[0] pt-[20px] right-[20px] shadow-lg rounded-[4px] overflow-hidden" onPointerEnter={() => setDropDownOpen(true)} onPointerLeave={() => setDropDownOpen(false)}>
